"use client";
import React, { ReactNode, useEffect, useState } from "react";
import Link from "next/link";
import {
  FiTrendingUp,
  FiSearch,
  FiSun,
  FiChevronDown,
  FiMoon,
  FiMenu,
  FiX,
} from "react-icons/fi";
import {
  Menu,
  MenuHandler,
  MenuList,
  MenuItem,
  IconButton,
  Avatar,
  Typography,
} from "@material-tailwind/react";
import {
  walsheim_bold,
  walsheim_light,
  walsheim_medium,
  walsheim_regular,
} from "@/components/constants";

import { notifications } from "./dashboard_data";
import { usePathname, useRouter, useSearchParams } from "next/navigation";
import { toast, Toaster } from "react-hot-toast";
import { getUser } from "@/utilities/getUser";
//------------- REDUX GTM ----------------
import { useSelector, useDispatch } from 'react-redux';
import { RootState } from '@/lib/store/store';
import { clearUserData } from '@/lib/store/features/auth/userSlice';
import { setFilter } from '@/lib/store/features/dashboard/filterSlice';
import Image from "next/image";

interface DashboardLayoutProps {
  children: ReactNode;
}

const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  const dispatch = useDispatch();
  const router = useRouter();
  const searchParams = useSearchParams();
  const pathname = usePathname();

  interface UserData {
    id: string | null;
    [key: string]: any;      // eslint-disable-line @typescript-eslint/no-explicit-any
  }

  const userData = useSelector((state: RootState) => state.auth?.user) as UserData;

  const [userID, setUserID] = useState<string | null>(null);
  const [fetchedUser, setFetchedUser] = useState<any>(null);      // eslint-disable-line @typescript-eslint/no-explicit-any
  const [, setLoading] = useState(true);
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [activeContent, setActiveContent] = useState<string | null>(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [activePath, setActivePath] = useState<string>("");
  const [t, setT] = useState(5)

  useEffect(() => {
    setActivePath(pathname);
  }, [pathname]);

  // const userFromLocalStorage = typeof window !== "undefined" ? localStorage.getItem("user") : null;

  // =========== userID from query OR localStorage GTM =================
  useEffect(() => {
    const paramID = searchParams.get("ID");
    const localID =  localStorage.getItem("USER_ID")?.slice(1, -1) || null;
    const resolvedUserID = paramID || localID || null;
    setUserID(resolvedUserID);

    if (!resolvedUserID) {
      console.error("* User ID not found! *");

      const interval = setInterval(() => {
        setT((t) => t > 0 ? t - 1 : 0)
      }, 1000);

      const timeout = setTimeout(() => {
        router.push("/auth/sign-in");
      }, 4550);  // ~ paanch second

      return () => {
        clearInterval(interval);
        clearTimeout(timeout);
      }
    }
  }, [searchParams]);

  
  useEffect(() => {
    if (userData) {
      setLoading(false);
      return;
    }

    if (userID) {
      // console.log("user id - ", userID, typeof userID)
      // console.log("localstorage - ", localStorage.getItem("USER_ID")?.slice(1,-1), typeof localStorage.getItem("USER_ID"))
      // console.log("userData - ", userData, typeof userData)
      getUser(userID)
        .then((res) => {
          setFetchedUser(res);
        })
        .catch((err) => {
          console.error(" Failed to fetch user:", err);
          setFetchedUser(null);
        })
        .finally(() => {
          setLoading(false);
        });
    } else {
      setLoading(false);
    }    
  }, [userID, userData]);

  // const activeUser = userData || (userFromLocalStorage ? JSON.parse(userFromLocalStorage) : null) || fetchedUser;
  const activeUser = userData || fetchedUser;


  if (!activeUser) return null
  else console.log("✅ Active User:", activeUser);

  if (!userID) {
    return (
      <div className="p-4 min-h-screen min-w-screen bg-black text-white text-sm font-semibold text-center flex flex-col justify-center items-center gap-4">
        <span className="border-4 border-white border-l-purple-600 animate-spin !duration-1000 !w-10 !h-10 rounded-full"></span>
        <p>Redirecting to sign-in page in {t} seconds...</p>
      </div>
    );
  }

  // ========================= LOGOUT GTM ============================
  const handleLogout = async () => {
    try {
      const response = await fetch("http://localhost:8000/api/v1/users/logout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      });
      if (!response.ok) {
        const errorData = await response.json();
        console.log("Error data - ", errorData)
        throw new Error(errorData?.message || "Logout failed.");
      }

      const data = await response.json();
      console.log("Logout Response:", data);
      dispatch(clearUserData());
      localStorage.removeItem("user");
      localStorage.removeItem("USER_ID");

      toast.success(data?.message || "Logged out successfully.");
      router?.push("/auth/sign-in");
    } catch (error: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
      console.error("Logout Error:", error);
      toast.error(`Failed to logout. ${error.message || error}`);

    }
  }

 // ========================== FILTER GTM =========================
  const handleFilter = (value: string) => {
    // setFilter(value);
    dispatch(setFilter(value));
    console.log("Selected filter:", value);
  }

  return (
    <>
      <Toaster />
<div suppressHydrationWarning={true}
  className={`flex h-screen max-h  bottom-0 ${isDarkMode ? "bg-white" : "bg-white"} overflow-hidden`}
>

        {/* Sidebar */}
        <aside
          className={`fixed lg:static z-30 w-64 bg-white dark:bg-gray-700 flex flex-col justify-between min-h-screen 
  transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} 
  transition-transform duration-300 lg:translate-x-0`}
        >
          <div>
            {/* Sidebar Header */}
            <div className="p-4 flex justify-between items-center dark:border-gray-700">
              <h1
                className={`text-xl sm:text-2xl font-bold dark:text-white truncate ${walsheim_bold.className}`}
              >
                ETERNITY LABS
              </h1>
              <button
                className="block lg:hidden text-gray-600 dark:text-gray-300"
                onClick={() => setIsSidebarOpen(false)}
              >
                <FiX size={20} />
              </button>
            </div>


            {/* Navigation */}
            <nav className="mt-4 ml-5 sm:mt-3">
              <ul className="space-y-0 sm:space-y-0">
                {[
                  {
                    key: "dashboard",
                    href: "/dashboard",
                    iconLight: "/icons/Dashboardicon.svg",
                    iconDark: "/icons/Dashboardicon-dark.svg",
                    label: "Dashboard",
                  },
                  {
                    key: "call-analytics",
                    href: "/dashboard/call-analytics",
                    iconLight: "/icons/Callicon.svg",
                    iconDark: "/icons/Callicon-dark.svg",
                    label: "Call Analytics",
                  },
                  {
                    key: "customer",
                    href: "/dashboard/customer",
                    iconLight: "/icons/Customericon.svg",
                    iconDark: "/icons/Customericon-dark.svg",
                    label: "Customer",
                  },
                  {
                    key: "feedback",
                    href: "/dashboard/feedback",
                    iconLight: "/icons/Feedbackicon.svg",
                    iconDark: "/icons/Feedbackicon-dark.svg",
                    label: "Feedback",
                  },
                  {
                    key: "follow-ups",
                    href: "/dashboard/follow-ups",
                    iconLight: "/icons/Followupsicon.svg",
                    iconDark: "/icons/Followupsicon-dark.svg",
                    label: "Follow-ups",
                  },
                  {
                    key: "leads",
                    href: "/dashboard/leads",
                    iconLight: "/icons/Leadsicon.svg",
                    iconDark: "/icons/Leadsicon-dark.svg",
                    label: "Leads",
                  },
                  {
                    key: "sources",
                    href: "/dashboard/sources",
                    iconLight: "/icons/Sourcesicon.svg",
                    iconDark: "/icons/Sourcesicon-dark.svg",
                    label: "Sources",
                  },
                  {
                    key: "Upload Recordings",
                    href: "/dashboard/Api-fetch",
                    iconLight: "/icons/API-light.png",
                    iconDark: "/icons/Api-dark.png",
                    label: "Upload Recordings",
                  },
                ].map(({ key, href, iconLight, iconDark, label }) => (
                  <li key={key}>
                    <Link
                      href={href}
                      className={`flex mr-6 items-center px-3 sm:px-4 py-2 sm:py-3 text-black rounded dark:text-gray-200 
                  ${activePath === href
                          ? "bg-[#EFE5FE] text-[#6F00DE] dark:bg-gray-800"
                          : "hover:bg-[#f8f5fc] dark:hover:bg-gray-600"
                        }`}
                      onClick={() => setActiveContent(key)}
                    >
                      <Image
                        width={20}
                        height={20}
                        src={isDarkMode ? iconDark : iconLight}
                        className="mr-2 sm:mr-3 text-sm sm:text-base"
                        alt={label}
                      />
                      <span
                        className={`text-sm ml-2 sm:text-base ${walsheim_medium.className}`}
                      >
                        {label}
                      </span>
                    </Link>
                  </li>
                ))}
              </ul>
            </nav>
          </div>

          {/* Settings Section */}
          <div className="p-3 sm:p-4">
            <Link
              href="/Owner_Settings"
              className={`flex items-center px-3 ml-1 sm:px-4 py-2 sm:py-3 text-black rounded dark:text-gray-200
      ${activeContent === "settings" ? "bg-[#EFE5FE] text-[#6F00DE] dark:bg-gray-800" : "hover:bg-[#f8f5fc] dark:hover:bg-gray-600"}`}
              onClick={() => setActiveContent("settings")}
            >
              <Image
                width={20}  
                height={20}
                src="/icons/settingsicon.svg"
                className="mr-2 sm:mr-3 text-sm sm:text-base dark:hidden"
                alt="Settings"
              />

              {/* Dark Mode Icon */}
              <Image
                width={20}  
                height={20}
                src="/icons/settingsicon-dark.svg"
                className="mr-2 sm:mr-3 text-sm sm:text-base hidden dark:block"
                alt="Settings (Dark Mode)"
              />
              <span className={`text-sm sm:text-base ${walsheim_medium.className}`}>
                Settings
              </span>
            </Link>
          </div>
        </aside>

        {/* Main Content */}
        <div className="flex-1 flex flex-col py-0 overflow-hidden dark:bg-gray-700">
          {/* Top Navigation */}
          <header className="flex items-center justify-between px-3 sm:px-4 md:px-6 py-2 sm:py-3  bg-white dark:bg-gray-700 overflow-x-auto">
            {/* Left Section */}
            <div className="flex items-center space-x-2 sm:space-x-3">
              <button
                className="block lg:hidden text-gray-600 dark:text-gray-300"
                onClick={() => setIsSidebarOpen(true)}
              >
                <FiMenu size={20} />
              </button>
              {/* <button className="flex items-center space-x-1 sm:space-x-2 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 dark:border-white border rounded-lg px-2 py-1 text-xs sm:text-sm">
                <span className={`${walsheim_medium.className}`}>Week</span>
                <FiChevronDown />
              </button> */}
              <select name="filter" id="filter" className={`flex items-center space-x-1 sm:space-x-2 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 dark:border-white border rounded-lg px-2 py-1 text-xs sm:text-sm cursor-pointer outline-none  ${walsheim_medium.className}`}
                onChange={(e) => handleFilter(e.target.value)}
              >
                <option value="hourly" >Hourly</option>
                <option value="daily">Daily</option>
                <option value="weekly">Weekly</option>
                <option value="monthly">Monthly</option>
                <option value="yearly">Yearly</option>
              </select>
              <button className="flex items-center space-x-1  sm:space-x-2 text-gray-600 dark:border-white hover:text-gray-800 border rounded-lg px-2 sm:px-3 py-[6px] text-xs sm:text-sm">
                <FiTrendingUp className="text-sm sm:text-base  dark:text-white" />
                <FiChevronDown className="text-gray-600 dark:text-white" />
              </button>
              <button className="text-gray-600 dark:text-white dark:border-white hover:text-gray-800 border text-center rounded-lg px-2 sm:px-3 py-1 text-xs sm:text-sm">
                ...
              </button>
            </div>

            {/* Center Section */}
            {/* Right Section */}
            <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
              {/* Search Bar - Moved before Dark Mode Toggle */}
              <div className="relative max-w-xs sm:max-w-sm md:max-w-md hidden sm:block">
                <FiSearch className="absolute left-2 sm:left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-300 text-xs sm:text-sm" />
                <input
                  type="text"
                  placeholder="Search"
                  className={`w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-1 sm:py-2 text-xs sm:text-sm bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-full focus:outline-none focus:ring focus:ring-purple-200 dark:focus:ring-purple-700 ${walsheim_medium.className}`}
                />
              </div>

              {/* Dark Mode Toggle */}
              <div
                className={`w-10 sm:w-12 h-5 sm:h-6 flex items-center rounded-full p-1 cursor-pointer ${isDarkMode ? "bg-gray-800" : "bg-gray-200"
                  }`}
                onClick={() => {
                  const newMode = !isDarkMode;
                  setIsDarkMode(newMode);
                  localStorage.setItem("dark-mode", newMode.toString());
                  if (newMode) {
                    document.documentElement.classList.add("dark");
                  } else {
                    document.documentElement.classList.remove("dark");
                  }
                }}
              >
                <div
                  className={`w-4 sm:w-5 h-4 sm:h-5 flex items-center justify-center rounded-full bg-white shadow-md transform transition-transform ${isDarkMode
                    ? "translate-x-5 sm:translate-x-6"
                    : "translate-x-0"
                    }`}
                >
                  {isDarkMode ? (
                    <FiMoon className="text-gray-800 dark:text-gray-800 text-xs sm:text-sm" />
                  ) : (
                    <FiSun className="text-yellow-500 text-xs sm:text-sm" />
                  )}
                </div>
              </div>

              <Menu
                animate={{
                  mount: { y: 0 },
                  unmount: { y: 25 },
                }}
              >
                <MenuHandler>
                  {/* @ts-expect-error: IconButton props type mismatch, usage is intentional */}
                  <IconButton variant="text" placeholder="">
                    <Image
                      width={24}
                      height={24}
                      src={isDarkMode ? "/icons/notification-icon-dark.svg" : "/icons/notification-icon.svg"}
                      alt="Notifications"
                      className="w-5 h-5"
                    />
                  </IconButton>
                </MenuHandler>
                {/* @ts-expect-error: MenuList props type mismatch, usage is intentional */}
                <MenuList
                  className="flex flex-col sm:w-4/12 bg-white dark:bg-black sm:-ml-32"
                  placeholder=""
                >
                  {notifications.map(
                    (notification: {
                      description: ReactNode;
                      id: React.Key | null | undefined;
                      name:
                      | string
                      | number
                      | bigint
                      | boolean
                      | React.ReactElement<
                        unknown,
                        string | React.JSXElementConstructor<any>       // eslint-disable-line @typescript-eslint/no-explicit-any
                      >
                      | Iterable<React.ReactNode>
                      | Promise<
                        | string
                        | number
                        | bigint
                        | boolean
                        | React.ReactPortal
                        | React.ReactElement<
                          unknown,
                          string | React.JSXElementConstructor<any>        // eslint-disable-line @typescript-eslint/no-explicit-any
                        >
                        | Iterable<React.ReactNode>
                        | null
                        | undefined
                      >
                      | null
                      | undefined;
                      avatar: string | undefined;
                      time:
                      | string
                      | number
                      | bigint
                      | boolean
                      | React.ReactElement<
                        unknown,
                        string | React.JSXElementConstructor<any>      // eslint-disable-line @typescript-eslint/no-explicit-any
                      >
                      | Iterable<React.ReactNode>
                      | React.ReactPortal
                      | Promise<
                        | string
                        | number
                        | bigint
                        | boolean
                        | React.ReactPortal
                        | React.ReactElement<
                          unknown,
                          string | React.JSXElementConstructor<any>    // eslint-disable-line @typescript-eslint/no-explicit-any
                        >
                        | Iterable<React.ReactNode>
                        | null
                        | undefined
                      >
                      | null
                      | undefined;
                    }) => (
                      // @ts-expect-error: MenuItem props type mismatch, usage is intentional
                        <MenuItem
                        key={notification.id}
                        className="flex items-center gap-4 py-2 pl-2 pr-8"
                        placeholder=""
                        >
                        {/* @ts-expect-error: Avatar props type mismatch, usage is intentional */}
                        <Avatar
                          variant="circular"
                          alt={notification.name as string | undefined}
                          src={notification.avatar as string | undefined}
                          placeholder={undefined}
                        />
                        <div className="flex flex-col gap-1">
                          {/* @ts-expect-error: Typography props type mismatch, usage is intentional */}
                          <Typography
                          variant="small"
                          color="gray"
                          placeholder=""
                          className={`text-black dark:text-white ${walsheim_medium.className}`}
                          >
                          {notification.name}{" "}
                          <span
                            className={`text-xs dark:text-gray-300 text-gray-500 ${walsheim_regular.className}`}
                          >
                            {notification.time}
                          </span>
                          </Typography>
                          <span
                          className={`text-gray-700 dark:text-white text-xs ${walsheim_light.className}`}
                          >
                          {notification.description}
                          </span>
                        </div>
                        </MenuItem>
                    )
                  )}
                </MenuList>
              </Menu>

              <div className="relative">

                {/* Profile Icon */}
                <div
                  className="flex items-center space-x-1 sm:space-x-2 cursor-pointer"
                  onClick={(e) => {
                    e.stopPropagation(); // Prevent event bubbling
                    setIsOpen(!isOpen);
                  }}
                >

                  {
                    (activeUser && activeUser?.avatar)
                      ?
                      <Image
                        width={24}    
                        height={24}
                        src={activeUser?.avatar}
                        alt="User avatar"
                        className="w-6 sm:w-8 h-6 sm:h-8 rounded-full"
                      />

                      :
                      <span className="w-6 sm:w-8 h-6 sm:h-8 rounded-full flex justify-center items-center font-bold text-sm text-white bg-purple-400  overflow-hidden">
                        {activeUser?.firstName?.[0].toUpperCase() + activeUser?.lastName?.[0].toUpperCase()}
                      </span>


                  }

                  <span
                    className={`text-xs sm:text-sm capitalize text-black dark:text-white ${walsheim_medium.className}`}
                  >
                    {activeUser?.firstName} {activeUser?.lastName}
                  </span>
                  <span className="text-xs sm:text-sm text-black dark:text-white">
                    <FiChevronDown />
                  </span>
                </div>

                {/* Dropdown */}
                {isOpen && (
                  <div
                    className="fixed top-16 right-4 w-64 bg-white shadow-lg rounded-lg p-4 z-50"
                    onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside
                  >
                    <h3 className="font-bold text-sm text-gray-800">Account</h3>
                    <div className="flex items-center space-x-3 mt-2">
                      <Link href={"/Owner_Settings"}>
                        {
                          (activeUser && activeUser?.avatar)
                            ?
                            <Image
                              width={24}  
                              height={24}
                              src={activeUser?.avatar}
                              alt="User avatar"
                              className="w-8 sm:w-10 h-8 sm:h-10 rounded-full"
                            />

                            :
                            <span className="w-8 sm:w-10 h-8 sm:h-10 rounded-full flex justify-center items-center font-bold text-sm text-white bg-purple-400  overflow-hidden">
                              {activeUser?.firstName[0].toUpperCase() + activeUser?.lastName[0].toUpperCase()}
                            </span>


                        }
                      </Link>
                      <div>
                        <p
                          className={`font-semibold capitalize dark:text-black text-sm ${walsheim_medium.className}`}
                        >
                          {activeUser?.firstName} {activeUser?.lastName}
                        </p>
                        <p className={`text-sm text-black ${walsheim_regular.className}`}>
                          {activeUser?.email}
                        </p>
                      </div>
                    </div>
                    <div className="mt-4 space-y-3">
                      <Link href={"/Owner_Settings"}>
                        <button
                          className={`flex items-center justify-between w-full text-sm font-semibold text-gray-800 ${walsheim_regular.className}`}
                        >
                          Manage Account
                          <span className="text-lg">
                            <Image src="/icons/dd.png" alt="" width={24} height={24} />
                          </span>
                        </button>
                      </Link>
                      <button
                        className={`flex items-center justify-between w-full text-sm font-semibold text-red-600 ${walsheim_regular.className}`}
                        type='button'
                        onClick={handleLogout}
                      >
                        Log Out
                        <span className="text-lg">
                          <Image src="/icons/log-out.png" alt="" width={24} height={24} />
                        </span>
                      </button>
                    </div>
                  </div>
                )}

                {isOpen && (
                  <div
                    className="fixed inset-0 z-40"
                    onClick={() => setIsOpen(false)}
                  ></div>
                )}
              </div>
            </div>
          </header>

          {/* Main Area */}
          <main className="flex-1 -top-1 relative p-3 sm:p-4 md:p-4 bg-[#EFE5FE] dark:bg-gray-800  md:rounded-tl-3xl overflow-auto">
            {/* Dynamic Content Based on Active Content */}
            {children}
            {/* {activeContent === "performance" && (
            <>
              <div className="grid grid-cols-1 gap-6 mt-2 lg:grid-cols-3">
                <RevenueCard data={SourcesData} />
                <PieChart
                  title="Agent Sentiments"
                  data={chartData3}
                />
              </div>
              <Chart4 />
          )} */}
            {/* {activeContent === "customer" && <Chart3 />} */}
            {/* {activeContent === "feedback" && <RevenueCard />} */}
          </main>
        </div>
      </div>
    </>
  );
};

export default DashboardLayout;
