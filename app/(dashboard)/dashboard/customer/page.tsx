...
const Page = () => {
  ...
  const fetchHourCustomers = async () => {
    try {
     ...
     const formattedData = data?.data?.map((item: any) => ({         // eslint-disable-line @typescript-eslint/no-explicit-any
        dateTime : new Date(item?.dateTime)?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        customerAcquired : item?.customerAcquired
      }));
      setHourlyCustomers(formattedData);
    } catch (error: any) {                    // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchHourTotalCustomers = async () => {
    try {
      ...
      const data = await response.json();
      console.log("→→ ======================== hourly total customers  data : ", data);
      setHourlyTotalCustomers(data?.data?.map((item: any) => item?.totalCustomers))       // eslint-disable-line @typescript-eslint/no-explicit-any
    } catch (error: any) {              // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchHourClosedCustomers = async () => {
    try {
      ...
      setHourlyClosedCustomers(data?.data.map((item: any) => item?.closedCustomers));     // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchMinAvgScore = async () => {
    try {
      ...
      const data = await response.json();
      console.log("*→ Minute avergage score : ", data);
      const formattedData = data?.data.map((item: any) => ({       // eslint-disable-line @typescript-eslint/no-explicit-any
      ...
      console.error("Error in fetching Minute avergae score:", error);
    }
  }
  ...
  return (
    ...
    <>
      <div className="grid grid-cols-1 gap-4 mt-2 lg:grid-cols-3">
                 {/* @ts-expect-error: data props type mismatch, usage is intentional */}
