...
const DefaultSelectOption = ({ options }: any) => {          // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  return (
    <ClickOutside onClick={() => setIsOpen(false)}>
      <div className="relative z-20 inline-flex cursor-pointer appearance-none rounded-[5px] border border-stroke bg-white text-sm font-medium outline-none dark:border-dark-3 dark:bg-dark-2">
        <div
          className={`py-[5px] pl-[9px] pr-[35px] text-sm font-medium text-dark dark:text-white dark:bg-black   ${isOpen ? "open" : ""}`}
        ...
        {isOpen && (
          <div className="absolute right-0 top-full z-40 mt-2 w-full rounded-[7px] border border-stroke bg-white dark:text-white dark:bg-black py-1.5 shadow-2 dark:border-dark-3 dark:bg-dark-2 dark:shadow-card">
            <ul>
