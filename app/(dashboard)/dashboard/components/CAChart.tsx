...
const CustomerAcquisitionChart: React.FC<PropsData> = ({dataValues}) => {
  const days = dataValues?.map((item:any) => item?.dateTime) || ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];       // eslint-disable-line @typescript-eslint/no-explicit-any
  // const dataPoints = [3500, 4200, 5000, 4890, 4600, 3800, 4000];
  const dataPoints = dataValues?.map((item : any) => item?.customerAcquired) || [3500, 4200, 5000, 4890, 4600, 3800, 4000];      // eslint-disable-line @typescript-eslint/no-explicit-any
