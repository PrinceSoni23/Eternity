...
const Page = () => {
  ...
  const [, setLifeTotalCalls] = useState([]);
  const [, setLifePickedCalls] = useState([]);
  ...
  const fetchTotalCalls = async () => {
    try {
      ...
      //console.log("-- Total call data : ", data?.data);
       const formattedData = data?.data?.map((item: any) => ({     // eslint-disable-line @typescript-eslint/no-explicit-any
        data: item?.totalCalls,
        label: new Date(item?.dateTime)?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }));
      setTotalCalls(formattedData);
       console.log("== Total calls data : ", formattedData);
  ...
  const fetchCallsPicked = async () => {
    try {
      ...
      const formattedData = data?.data?.map((item: any) => ({       // eslint-disable-line @typescript-eslint/no-explicit-any
        data: item?.callPickedNumber,
        label: new Date(item?.dateTime)?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }));
      console.log("== Picked calls data : ", formattedData);
      setCallsPicked(formattedData);
    ...
    <>
      ...
      <>
        <div className="grid grid-cols-1 gap-4 mt-2 lg:grid-cols-3">
          ...
          />
          <RevenueCard 
          head1="Total Calls" data1={totalCalls.map( (x:any) => x.data)}       // eslint-disable-line @typescript-eslint/no-explicit-any
          head2="Picked Calls" data2={callsPicked.map( (x:any) => x.data)}      // eslint-disable-line @typescript-eslint/no-explicit-any
          />
          <RevenueCard 
          head1="Average Hold Time" data1={hourAvgHoldTime.map( (x:any) => x.data)}   // eslint-disable-line @typescript-eslint/no-explicit-any
          head2="Average Air Time" data2={hourAvgAirTime.map( (x:any) => x.data)}      // eslint-disable-line @typescript-eslint/no-explicit-any
          />
        </div>
        <ChartCall4
          totalCalls={totalCalls}
          pickedCalls={callsPicked}
          durationData={hourTotalDuration}
          holdTimeData={hourAvgHoldTime}
          airTimeData={hourAvgAirTime}
        />
      </>
