...
const Page = () => {
  ...
  const [, setImpressionData] = useState<any>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
  const [uniqueImpressionData, setUniqueImpressionData] = useState<any>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
  const [totalImpressionData, setTotalImpressionData] = useState<any>([]);  // eslint-disable-line @typescript-eslint/no-explicit-any
  const [, setTotalTransactions] = useState<any>([]);     // eslint-disable-line @typescript-eslint/no-explicit-any
  const [totalRevenue, setTotalRevenue] = useState<any>([]);    // eslint-disable-line @typescript-eslint/no-explicit-any
  const [allTransactions, setAllTransactions] = useState<any>([]);    // eslint-disable-line @typescript-eslint/no-explicit-any
  const [individualTotalTxns, setIndividualTotalTxns] = useState<any>([]);  // eslint-disable-line @typescript-eslint/no-explicit-any
  const [totalPeopleConnected, setTotalPeopleConnected] = useState<any>([]);  // eslint-disable-line @typescript-eslint/no-explicit-any
  const [allConnections, setAllConnections] = useState<any>([]);   // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchUniqueImpressionData = async () => {
    try {
      ...
      //console.log("Unique Impressions data : ", data?.data);
      // const formattedImpressionData = data?.data?.map((item: any) => {
      //   return {
      //     label: formatLabel(item?.dateTime, filter),
      //     data: item?.uniqueImpressions,
      //   };
      // }
      // )
      // console.log(" FORMATTED IMPRESSION DATA : ", formattedImpressionData);
      setUniqueImpressionData(data?.data);
    } catch (error: any) {     // eslint-disable-line @typescript-eslint/no-explicit-any
      console.error("Error in fetching impressions data:", error);
    }
  }
  const fetchTotalImpressionData = async () => {
    try {
      ...
      const data = await response.json();
      // setImpressionData(data.data);
      //console.log("Total Impressions data : ", data);
      setTotalImpressionData(data?.data?.map((item: any) => item?.totalImpressions));   // eslint-disable-line @typescript-eslint/no-explicit-any
    } catch (error: any) {     // eslint-disable-line @typescript-eslint/no-explicit-any
      console.error("Error in fetching impressions data:", error);
    }
  }
  ...
  const fetchAllTransactions = async () => {
    try {
      ...
      // const formattedTxnData = data?.data?.map((txn: any) => {
      //   return {
      //     label: formatLabel(txn?.dateTime, filter),
      //     data: txn?.amount,
      //   };
      // })
  ...
  const fetchTotalPeopleConnected = async () => {
    try {
      ...
      const data = await response.json();
      //console.log("→ TOTAL PEOPLE CONNECTED : ", data?.data);
      setTotalPeopleConnected(data?.data);
    } catch (error: any) {   // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    <>
      <div className="grid grid-cols-1 gap-4 mt-2 lg:grid-cols-3">
        <RevenueCard
          ...
          head2='Unique Impressions' data2={formattedUniqueImpressionData?.map((item: any) => item?.data)}  // eslint-disable-line @typescript-eslint/no-explicit-any
        />
