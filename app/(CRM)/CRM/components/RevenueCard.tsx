...
const RevenueCard: React.FC<RevenueCardProps> = ({ data }) => {
  return (
    <div className="bg-white rounded-[40] flex items-center justify-center dark:bg-gray-800 dark:border-gray-700  dark:shadow-gray-600 border-gray-100 shadow-md border">
      {/* Card Container */}
      <div className="bg-white dark:bg-gray-800 rounded-[40] p-6 w-full max-w-md space-y-6">
        {data.map((item, index) => (
          <div key={index}>
            <div className="flex items-center space-x-4">
              ...
              <div>
                 ...
                 <p className={`text-gray-500 dark:text-white text-sm ${walsheim_regular.className}`}>{item.description}</p>
              </div>
            </div>
            {/* Divider for items */}
            {index < data.length - 1 && (
